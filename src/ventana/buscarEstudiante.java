...
public class buscarEstudiante extends JFrame {
	...
	public buscarEstudiante() {
		...
		btnBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					// valido que el usuario no dijite un codigo vacio:
					if (textCodigo.getText().isEmpty()) {
						throw new Exception();
					}
					// creo una lista para guardar al objeto que se busca:
					ArrayList<Estudiante> estudiantes = new ArrayList<>();
					String codigo = textCodigo.getText();
					Scanner scanner = new Scanner(new File("estudiantes.ser"));
					// recorro el archivo para buscar al objeto:
					while (scanner.hasNextLine()) {
						...
						// una vez encuentro el objeto lo guardo en la lista:
					...
					// si el estudiante existe genero una nueva ventana y la lleno con los datos del
					// estudiante:
					if (estudiantes.size() == 1) {
						editarEstudiante nuevaVentana = new editarEstudiante(estudiantes.get(0).getNotaDefinitiva(),
								estudiantes.get(0).getCodigo());
					...
					// en caso de que no exista genero una ventana emergente:
					else {
					...
					JOptionPane.showMessageDialog(null, "porfavor digita un codigo valido");
		...
		lblFondo.setIcon(new ImageIcon(new ImageIcon(ventana.class.getResource("/imagenes/fondo.png")).getImage()
				.getScaledInstance(434, 167, Image.SCALE_DEFAULT)));
